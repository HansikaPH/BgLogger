...
public class ReportDataManipulator
{
    ...
    public List<String[]> selectEmail(){
    	List<String[]> list = new ArrayList<String[]>();
        Cursor cursor = db.query(TABLE_NAME, new String[] { "name","address" }, "address IS NOT NULL", null, null, null, "name asc");
        int x=0;
        if (cursor.moveToFirst()) {
           do {
                String[] b1=new String[]{cursor.getString(cursor.getColumnIndex("name")),cursor.getString(cursor.getColumnIndex("address"))};
                list.add(b1);
                x=x+1;
           } while (cursor.moveToNext());
        }
